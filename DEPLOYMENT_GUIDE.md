...
**Local development:** use the centralized `run` command and catalog in `C:/Noma/system/config/`. Developer guide: **`C:/Noma/system/docs/GUIA_DESENVOLVEDOR.md`** (PT-BR) or `ENVIRONMENT_SETUP.md` (EN). Grammar: `run <apps> env:<staging|prod> [handler:<local|staging|prod>]`. Generated files: `env_config.py`, `NOMA/.env.local`, `console/.env.development`.

### Frontend (NOMA app)

Main variables (see `C:/Noma/system/config/env.catalog.yaml`; reference stub `C:/Noma/NOMA/env.example`):
...
Main variables (generated by `run` into `.env.development`; legacy templates are deprecated):
...
**Local:** Use `run backend env:staging handler:local` (or `env:prod`) to generate `env_config.py` from `system/config/` + AWS Secrets Manager. Manual copy from `env_config.py.TEMPLATE` is legacy-only.
